...
function ScanPage() {
  ...
  const [nickname, setNickname] = useState("Kitchen cylinder");
  ...
  async function handleCapture(dataUrl: string) {
    ...
    setResult(null);
    try {
      const r = (await analyzeCylinderImage({ data: { imageDataUrl: dataUrl } })) as ScanResult;
      setResult(r);
    } catch (e) {
  ...
  function save() {
    ...
    // best-effort push notification scheduling
  ...
  return (
    <AppShell title="Scan cylinder" subtitle="Frame the metal test ring inside the box.">
      ...
      {result && "error" in result && (
        <div className="rounded-2xl border border-border bg-card p-6 text-center status-stripe-warn">
          ...
          <p className="font-semibold">Couldn't read the code</p>
          <p className="text-sm text-muted-foreground mt-1">Try better lighting or move closer to the ring.</p>
      ...
      {result && (
        <button onClick={reset} className="mt-4 inline-flex items-center gap-1 text-sm text-muted-foreground">
          <ArrowLeft className="h-4 w-4" /> Rescan
        </button>
      )}
    </AppShell>
  );
...
function ResultCard({
  ...
  const message = result.is_expired
    ? "This cylinder is expired. Do not use it — request a replacement immediately."
    : s === "warn"
      ? `Cylinder is safe but expiring in ${result.months_remaining} month${result.months_remaining === 1 ? "" : "s"}.`
      : `Your cylinder is safe until ${result.expiry_date}.`;

  return (
    <div className={`rounded-2xl bg-card p-5 ${stripe}`}>
      <div className="flex items-start justify-between">
        <div>
          <p className="text-xs uppercase tracking-widest text-muted-foreground">Expiry code</p>
      ...
      <label className="mt-5 block">
        <span className="text-xs font-semibold uppercase tracking-wide text-muted-foreground">Nickname</span>
        <input
          value={nickname}
      ...
      >
        {saved ? "Saved ✓" : "Save to my cylinders"}
      </button>
    </div>
  );
}
...
async function scheduleExpiryReminder(nickname: string, monthsRemaining: number) {
  ...
  try {
    ...
    // Approximate: notify when 30 days remain. setTimeout caps at ~24.8d; we set it only if within range.
